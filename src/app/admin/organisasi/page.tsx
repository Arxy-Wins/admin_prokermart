...
export default function OrganisasiPage() {
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    try {
      ...
      setOrganisasiList((prev) =>
        prev.map((t) =>
          t.id_organisasi === id ? { ...t, status_verifikasi: status } : t,
        ),
      );
    } catch (err) {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    e.preventDefault();
    setInviteError("");
    if (!form.nama_toko.trim() || !form.email.trim()) {
  ...
  const filtered = OrganisasiList.filter((t) => {
    ...
    const matchSearch = t.nama_organisasi
      .toLowerCase()
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {["Nama Organisasi", "Status", "Tgl. Daftar", "Aksi"].map(
                    (h) => (
                      <th
                        key={h}
                  ...
                  filtered.map((t) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-[11px] font-bold px-2 py-1 rounded-full ${STATUS_BADGE[t.status_verifikasi] ?? "bg-slate-100 text-slate-600"}`}
                        >
                          {STATUS_LABEL[t.status_verifikasi] ??
                            t.status_verifikasi}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-slate-500 text-xs">
                        {new Date(t.tgl_daftar).toLocaleDateString("id-ID")}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          ...
                          {t.status_verifikasi !== "suspended" && (
                            <button
                              onClick={() =>
                                updateStatus(t.id_organisasi, "suspended")
                              }
                              disabled={actionLoading === t.id_organisasi}
            ...
            >
              ...
              {inviteSuccess ? (
                <div className="text-center py-6 space-y-3">
                  ...
                  <p className="text-sm text-slate-500">
                    Email undangan dikirim ke <strong>{form.email}</strong>.
                    Mereka perlu klik link di email untuk menyelesaikan
                    pendaftaran.
                  </p>
                  <button
                    onClick={closeModal}
                    className="mt-2 bg-blue-600 text-white font-bold px-6 py-2 rounded-xl text-sm hover:bg-blue-700 transition-colors"
                  >
                    Tutup
                  </button>
                </div>
                ...
                <form onSubmit={handleInvite} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({ ...f, nama_toko: e.target.value }))
                      }
                  ...
                  <p className="text-xs text-slate-400">
                    Email undangan akan dikirim ke alamat di atas. Penerima
                    perlu klik link untuk menyelesaikan pendaftaran toko.
                  </p>
